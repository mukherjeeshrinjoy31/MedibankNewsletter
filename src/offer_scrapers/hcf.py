...
def run(local: Optional[str] = None):
    ...
    if local:
        save_locally(payload, SOURCE.HCF.value, DATASET.COMPETITOR_OFFERS.value, local)
        update_excel(structured_offers, BRAND, merge_fn=merge_hcf_offers, local_dir=local)
    else:
